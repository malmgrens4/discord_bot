...
match_league_url = "https://americas.api.riotgames.com"
...
def get_match_results(game_id):
    return requests.get(match_league_url + '/lol/match/v5/matches/NA1_' + str(game_id) + '?api_key=' + league_api_key)


def get_summoner_and_puuid_id_from_username(league_name):
    r = requests.get(
        league_url + '/lol/summoner/v4/summoners/by-name/' + league_name + '?api_key=' + league_api_key)
    sum_data = r.json()
    summoner_id = sum_data["id"]
    puuid_id = sum_data["puuid"]
    return summoner_id, puuid_id


def update_summoner_and_puuid_ids():
    for user in db_api.get_users_all():
        if user.league_name:
            summoner_id, league_puuid = get_summoner_and_puuid_id_from_username(user.league_name)
            db_api.update_summoner_id_and_puuid(user.id, summoner_id, league_puuid)
